...
/**
 * {@link IConfigBuilder} implementation wrapping Forge 1.7.10's
 ...
 * {@link ForgeConfigValue} holding the result.
 * <p>
...
public class ForgeConfigBuilder implements IConfigBuilder {
    ...
    public ForgeConfigBuilder(Configuration configuration) {
    ...
    public IConfigValue<Integer> defineInt(String key, int defaultValue, int min, int max, String comment) {
        return new ForgeConfigValue<>(configuration.getInt(key, category(key), defaultValue, min, max, comment));
    }

    @Override
    public IConfigValue<String> defineString(String key, String defaultValue, String comment) {
        return new ForgeConfigValue<>(configuration.getString(key, category(key), defaultValue, comment));
    }

    @Override
    public IConfigValue<Boolean> defineBoolean(String key, boolean defaultValue, String comment) {
        return new ForgeConfigValue<>(configuration.getBoolean(key, category(key), defaultValue, comment));
    }
}
