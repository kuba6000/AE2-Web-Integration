        ...
        if (type == ERequestType.CHECK) {
                ...
                try {
                    ...
                    for (IItemStack stack : plan) {
                        ...
                        if (jobItem.missing == 0 && jobItem.requested == 0 && jobItem.stored > 0) {
                            IItemStack realStack = items.getAvailableItem(stack, grid);
                            long available = 0L;
            ...
            if (job.isDone()) {
                try {
                    ...
                    ITextComponent error = craftingGrid.submitJob(craftingJob, target, true, grid);
                    if (error != null) {
                        deny("FAIL");
                        setData(error.getUnformattedText());
                    } else {
                        done();
                    }
                } catch (InterruptedException | ExecutionException e) {
